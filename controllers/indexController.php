...
class indexController extends \Bang\SuperController implements \Bang\ControllerInterface
{
    ...
    public function __construct(\stdClass $di)
    {
    	...
    	$this->View->addStyle($this->View->TemplatePath.'min/css/dashboard.min.css', 0);
    	$this->View->addScript($this->View->TemplatePath.'min/js/dashboard.js', 0);
    ...
    public function indexAction() {
    	...
    	$dbMessages  = $this->Data->getSystemMessages('db');
    	$appMessages = $this->Data->getSystemMessages('app');
    	
    	$this->View->setTplVar('dbMessages', $dbMessages);
    	$this->View->setTplVar('appMessages', $appMessages);
    	 
    	if(file_exists($this->Overwrite.'dashboard'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'dashboard.php')) {
    		$template = $this->View->loadTemplate($this->Overwrite.'dashboard'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'dashboard.php');
    	} else {
    		$template  = $this->View->loadTemplate($this->path.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'dashboard.php');
    	}
    	 
    	// main template
    	$this->View->setModuleTpl('dashboard', $template);
